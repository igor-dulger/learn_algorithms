...
class FlowEdge(object):
    ...
    def other(self, v):
        return self.__from if self.__to == v else self.__to
    ...
    def residual_capacity_to(self, v):
        return self.__capacity - self.__flow if v == self.__to else self.__flow
    ...
    def __repr__(self):
        return "{}->{}({}/{})".format(self.__from, self.__to, self.__flow, self.__capacity)
...
class FlowNetwork(object):
    ...
    def __load_from_file(self, file):
        ...
        for line in [i.strip().split() for i in file]:
            if len(line) == 3:
                self.add_edge(FlowEdge(int(line[0]), int(line[1]), line[2]))
        file.close()
